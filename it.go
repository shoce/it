...
release:
git init
git add it.go readme.text
git commit -am `{it v}
git branch -M main
git remote add origin git@github.com:shoce/it.git
git push -f -u origin main
rm -rf .git
rm -f it.linux.gz it.macos.gz
GOOS=linux GOARCH=amd64 go build -trimpath -o it.linux
GOOS=darwin GOARCH=amd64 go build -trimpath -o it.macos
gzip it.linux it.macos
echo https://github.com/shoce/it/releases
it v
open .
